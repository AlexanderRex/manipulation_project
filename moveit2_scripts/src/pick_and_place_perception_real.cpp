...
private:
  ...
  void result_callback(const GoalHandleFind::WrappedResult &result) {
    ...
    for (auto object : result.result->objects) {
      ...
      x = object.object.primitives[0].dimensions[0];
      y = object.object.primitives[0].dimensions[1];
      z = object.object.primitives[0].dimensions[2];
    }
...
int main(int argc, char **argv) {
  ...
  target_pose1.position.x = action_client->x + 0.318436; // calibration number
  target_pose1.position.y = action_client->y + 0.118178;
  target_pose1.position.z = action_client->z + 0.186304;
